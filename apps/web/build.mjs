...
await mkdir(path.join(vendor, 'antenna'), { recursive: true });
...
for (const f of antennaFiles) {
  await cp(path.join(antennaSrc, f), path.join(vendor, 'antenna', f));
}
console.log(`antenna model: ${antennaFiles.length} modules`);
await writeFile(path.join(vendor, 'antenna.mjs'),
  "// Generated by apps/web/build.mjs. Do not edit; do not commit.\nexport * from './antenna/index.mjs';\n");
