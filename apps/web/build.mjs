...
const ANTENNA_DIR = 'antenna-model';
await mkdir(path.join(vendor, ANTENNA_DIR), { recursive: true });
const antennaSrc = path.join(repo, 'packages/antenna');
const antennaFiles = (await readdir(antennaSrc)).filter((f) => f.endsWith('.mjs'));
if (!antennaFiles.includes('index.mjs')) throw new Error('packages/antenna has no index.mjs');
for (const f of antennaFiles) {
  await cp(path.join(antennaSrc, f), path.join(vendor, ANTENNA_DIR, f));
}
console.log(`antenna model: ${antennaFiles.length} modules in vendor/${ANTENNA_DIR}/`);
await writeFile(path.join(vendor, `${ANTENNA_DIR}.mjs`),
  "// Generated by apps/web/build.mjs. Do not edit; do not commit.\n"
  + `export * from './${ANTENNA_DIR}/index.mjs';\n`);
