...
function draw(){
  ...
  ctx.strokeStyle=cssv('--hf-text-muted'); ctx.lineWidth=1; ctx.globalAlpha=.55; ctx.beginPath();
  ...
  // circuits to each receiver, coloured by how that circuit performs
  ...
  for(const rx of receivers){
    ...
    ctx.save();
    ctx.strokeStyle=statusFill(tok); ctx.lineWidth=3; ctx.globalAlpha=.95;
    ...
    const rxx=px(rx.lon), rxy=py(rx.lat);
    ctx.fillStyle=statusFill(tok); ctx.strokeStyle=cssv('--hf-text-primary'); ctx.lineWidth=1.5;
    ctx.beginPath(); ctx.arc(rxx,rxy,6,0,Math.PI*2); ctx.fill(); ctx.stroke();
    ctx.restore();
  }
